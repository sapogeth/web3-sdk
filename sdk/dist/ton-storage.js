...
export class TonStorageClient {
    ...
    async registerKeys(_keys, signedBoc) {
        await this.sendBoc(signedBoc);
    }
    async fetchKeys(address) {
    ...
    async deleteMessage(_address, _messageId, signedBoc) {
        await this.sendBoc(signedBoc);
    }
    // ── Build transaction bodies for TonConnect signing ───────────────────────
    buildRegisterKeysBody(keys) {
    ...
    buildStoreMessageBody(toAddress, bagId) {
        const bits = packUint(OP_STORE_MESSAGE, 32) +
            packUint(0, 64) +
            packAddr(toAddress) +
            packUint(BigInt('0x' + bagId.padEnd(64, '0').slice(0, 64)), 256);
        return encodeCellHex(bits, []);
    }
    buildDeleteMessageBody(bagId) {
        const bits = packUint(OP_DELETE_MESSAGE, 32) +
            packUint(0, 64) +
            packUint(BigInt('0x' + bagId.padEnd(64, '0').slice(0, 64)), 256);
        return encodeCellHex(bits, []);
    }
    ...
    async runGetMethod(method, stack) {
        ...
        if (!res.ok)
            throw new TonStorageError(`runGetMethod ${method} failed: ${res.status}`, res.status);
    ...
    async sendBoc(bocBase64) {
        ...
        if (!res.ok)
            throw new TonStorageError(`sendBoc failed: ${res.status}`, res.status);
    }
    async downloadBag(bagId) {
        // Try TON Storage HTTP API
        const res = await this.fetch(`/storage/torrent?bag_id=${encodeURIComponent(bagId)}`, { method: 'GET' }).catch(() => null);
        if (!res?.ok)
            return null;
        try {
...
function stackCellToBase64(item) {
    if (!item || item[0] !== 'cell')
        return '';
...
function stackCellChainToBase64(item, _expectedBytes) {
...
function cellBytesToBase64(hex) {
    if (!hex)
        return '';
...
function parseMsgMetaDict(rawStack) {
    ...
    if (!cellData || typeof cellData !== 'object')
        return [];
    const entries = (cellData['entries'] ?? cellData['dict'] ?? []);
    return entries
...
function buildDataCell(data) {
    // Simple cell: data bits only, no refs
    let bits = '';
    for (const byte of data)
        bits += packUint(byte, 8);
    return bytesToHex(bitsToBytes(bits));
}
function buildMlkemCellChain(ekBytes) {
...
function encodeCellHex(bits, _refs) {
...
async function computeBagId(to, msg) {
    const data = new TextEncoder().encode(to + msg.from + msg.timestamp.toString() + msg.ciphertext.slice(0, 32));
